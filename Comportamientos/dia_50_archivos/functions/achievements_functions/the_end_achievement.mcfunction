...
execute @s[tag=!end] ~ ~ ~ tellraw @a {"rawtext":[{"translate":"achievements.text","with":{"rawtext":[{"selector":"@s"},{"translate":"achievements.the_end"},{"text":""}]}}]}
execute @s[tag=!end] ~ ~ ~ tag @s add end
execute @s[tag=in_end] ~ ~ ~ detect ~ ~-1 ~ bedrock 0 effect @s levitation 9 12
execute @s[tag=in_end] ~ ~ ~ detect ~ ~-1 ~ bedrock 1 effect @s levitation 9 12
execute @s[tag=in_end] ~ ~ ~ fill ~24 ~4 ~24 ~-24 ~-4 ~-24 air 0 replace redstone_block