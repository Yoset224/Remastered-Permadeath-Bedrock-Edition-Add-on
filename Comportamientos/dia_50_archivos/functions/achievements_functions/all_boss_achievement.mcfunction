...
execute @s[tag=bossone,tag=bosstwo,tag=bossthree,tag=!hunterboss] ~ ~ ~ tellraw @a {"rawtext":[{"translate":"achievements.text_complete","with":{"rawtext":[{"selector":"@s"},{"translate":"achievements.master_boss"},{"text":""}]}}]}
execute @s[tag=bossone,tag=bosstwo,tag=bossthree,tag=!hunterboss] ~ ~ ~ playsound ui.achievement.complete @s
execute @s[tag=bossone,tag=bosstwo,tag=bossthree,tag=!hunterboss] ~ ~ ~ tag @s add hunterboss