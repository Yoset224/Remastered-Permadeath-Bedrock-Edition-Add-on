...
execute @s[tag=!generation] ~ ~ ~ tellraw @a {"rawtext":[{"translate":"achievements.text_complete","with":{"rawtext":[{"selector":"@s"},{"translate":"achievements.generation"},{"text":""}]}}]}
execute @s[tag=!generation] ~ ~ ~ playsound ui.achievement.complete @s
execute @s[tag=!generation] ~ ~ ~ tag @s add generation