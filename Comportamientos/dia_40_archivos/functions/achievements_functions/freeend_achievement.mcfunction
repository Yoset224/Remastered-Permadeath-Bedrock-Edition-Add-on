...
execute @s[tag=!freeend] ~ ~ ~ tellraw @a {"rawtext":[{"translate":"achievements.text","with":{"rawtext":[{"selector":"@s"},{"translate":"achievements.free_to_the_end"},{"text":""}]}}]}
execute @s[tag=!freeend] ~ ~ ~ tag @s add freeend