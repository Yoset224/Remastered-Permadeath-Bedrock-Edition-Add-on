...
execute @s[tag=!explorer,tag=vj1,tag=vj2,tag=vj3] ~ ~ ~ tellraw @a {"rawtext":[{"translate":"achievements.text_complete","with":{"rawtext":[{"selector":"@s"},{"translate":"achievements.explorer"},{"text":""}]}}]}
execute @s[tag=!explorer,tag=vj1,tag=vj2,tag=vj3] ~ ~ ~ playsound ui.achievement.complete @s
execute @s[tag=!explorer,tag=vj1,tag=vj2,tag=vj3] ~ ~ ~ tag @s add explorer